...
(function ($, learun) {
    ...
    var page = {
        init: function () {
            ...
            // 退出按钮
            $('#lr_exit_btn').on('click', function () {
            	$("#lr_loginout_btn").trigger("click");
            });

            //待办按钮
            $('#lr_task_btn').on('click', function () {
            	$("#lr_loginout_btn").trigger("click");
            });

            //修改密码按钮
            $('#lr_password_btn').on('click', function () {
                learun.layerForm({
                    id: 'form',
                    title: '修改密码',
                    url: top.$.rootUrl + '/UserCenter/PassWordForm',
                    width: 450,
                    height: 320,
                    btn:null,
                    maxmin: true,
        ...
        uitheme: function () {
            ...
            _html += '<div><label><input type="radio" name="ui_theme" value="5" ' + (uitheme == '4' ? 'checked' : '') + '>矿业版</label></div>';
            _html += '</div>';
            $setting.append($btn);
