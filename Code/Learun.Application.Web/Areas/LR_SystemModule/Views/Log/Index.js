...
var bootstrap = function ($, learun) {
    ...
    var page = {
        ...
        bind: function () {
            ...
            $('#lr_removelog').on('click', function () {
                learun.layerForm({
                    id: 'form',
                    title: '清空',
                    url: '/LR_SystemModule/Log/Form?categoryId=' + categoryId,
                    height: 200,
                    width: 400,
                    callBack: function (id) {
                        return window.parent[id].acceptClick(refreshGirdData);
                    }
                });
            });
            // 旧版导出
            ...
            //        height: 380,
            //        callBack: function (id) {
            //            return window.parent[id].acceptClick();
            //        },
            //        btn: ['导出Excel', '关闭']
            //    });
            //});
            ...
            // 新版导出
            $('#lr_export').on('click', function () {
                let op = $('#gridtable').jfGridGet('settingInfo');
                var $form = $("<form action='/LR_SystemModule/Log/ExportExcel' method='post'></form>");
                var $input = $('<input type="hidden" />').attr('name', 'queryJson').val(op.param.queryJson);
                $form.append($input);
        ...
        initGrid: function () {
            $('#gridtable').jfGrid({
                url: '/LR_SystemModule/Log/GetPageList',
                headData: [
                     {
